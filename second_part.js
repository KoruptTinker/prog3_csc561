...
const INPUT_TEXTURES_URL = "https://ncsucgclass.github.io/prog4/"; // textures file loc
...
function setupWebGL() {
  ...
  gl = canvas.getContext("webgl", { alpha: true }); // get a webgl object with alpha channel
  ...
  try {
    if (gl == null) {
      ...
      gl.clearColor(0.0, 0.0, 0.0, 0.0); // use transparent when we clear the frame buffer
      gl.clearDepth(1.0); // use max when we clear the depth buffer
      gl.enable(gl.DEPTH_TEST); // use hidden surface removal (with zbuffering)
      gl.enable(gl.BLEND); // enable blending for transparency
      gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA); // standard blending
    }
  } // end try
...
function renderTriangles() {
  ...
  for (var itr = 0; itr < TriangleSetInfo.length; itr++) {
    ...
    if (textureArray[itr]) {
      gl.bindTexture(gl.TEXTURE_2D, textureArray[itr]);
    }
...
function updateViewingCoordinates() {
  ...
  // Recalculate angles based on new eye and target positions
